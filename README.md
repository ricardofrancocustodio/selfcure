...
| Command | Description |
|---------|-------------|
| `selfcure init` | Scaffold `selfcure.config.js` in the current project |
...
├── generator     component analysis → Claude → Playwright .spec.ts
├── runner        @playwright/test programmatic API + trace capture
├── selfcure      self-healing loop (Claude diff → patch → re-run)
...
```
cli ──► crawler ──► analyzer ──► generator ──► [Claude]
                                      │
         runner ◄────────────────────┘
           │
         selfcure ──► [Claude]
...
| [Configuration reference](docs/configuration.md) | All `selfcure.config.js` options |
